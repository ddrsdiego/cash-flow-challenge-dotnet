...
public static class IngestTransactionsBatchErrors
{
    private const string Instance = "/IngestTransactionsBatch";

    // ═══════════════════════════════════════════════════════════════════════════
    // ERROS 400 - Input inválido (SEM RETRY)
    // ═══════════════════════════════════════════════════════════════════════════

    public static Response EmptyTransactions(string tracerId) =>
        Response.Builder()
            ...
            .WithErrorResponse(
                ErrorResponse.Builder()
                    ...
                    .WithError("EMPTY_TRANSACTIONS", "EMPTY_TRANSACTIONS", "Batch must contain at least one transaction")
                    .Build())
            .Build();
    ...
    public static Response PublishError(string tracerId, string error) =>
        Response.Builder()
            .WithRequestId(tracerId)
            .WithStatusCode(StatusCodes.Status500InternalServerError)
            .WithErrorResponse(
                ErrorResponse.Builder()
                    .WithInstance(Instance)
                    .WithTraceId(tracerId)
                    .WithError("PUBLISH_ERROR", "PUBLISH_ERROR", $"Message publish error: {error}")
                    .Build())
            .Build();
