...
public sealed class IngestTransactionsBatchCommandHandler :
    IRequestHandler<IngestTransactionsBatchCommand, Response>
{
    ...
    private async Task<Response> PersistBatchAsync(IngestTransactionsBatchCommand request,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await _repository.BulkInsertAsync(receivedTransactions, _transactionalPublisher.Session, cancellationToken);

            var consolidationBatchReceivedEvent = new ConsolidationBatchReceivedEvent(request.BatchId, request.TracerId);
